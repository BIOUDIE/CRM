      ...
      <div className="w-full max-w-5xl grid lg:grid-cols-2 gap-12 items-center relative z-10">
        <div className="hidden lg:block space-y-8 text-left">
          ...
          <h1 className="text-6xl font-bold text-white leading-tight">
            Manage <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">Clients</span> with Vibe.
          </h1>
          <p className="text-xl text-slate-400 max-w-md">The CRM designed for freelancers who care about relationship health.</p>
        </div>

        <div className="bg-slate-900/40 backdrop-blur-2xl border border-slate-800 rounded-[2.5rem] p-8 lg:p-12 shadow-2xl text-left">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {mode === 'signup' && (
              <div className="group relative">
                ...
                <input type="text" placeholder="Full Name" required className="w-full bg-slate-800/40 border border-slate-700/50 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-blue-500" onChange={(e) => setFormData({...formData, name: e.target.value})} />
              </div>
            )}
            <div className="group relative">
              <Mail className="absolute left-4 top-4 w-5 h-5 text-slate-500" />
              <input type="email" placeholder="Email address" required className="w-full bg-slate-800/40 border border-slate-700/50 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-blue-500" onChange={(e) => setFormData({...formData, email: e.target.value})} />
            </div>
            <div className="group relative">
              <Lock className="absolute left-4 top-4 w-5 h-5 text-slate-500" />
              <input type="password" placeholder="Password" required className="w-full bg-slate-800/40 border border-slate-700/50 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-blue-500" onChange={(e) => setFormData({...formData, password: e.target.value})} />
            </div>
            {mode === 'signup' && (
              <div className="group relative">
                <CheckCircle className="absolute left-4 top-4 w-5 h-5 text-slate-500" />
                <input type="password" placeholder="Confirm Password" required className="w-full bg-slate-800/40 border border-slate-700/50 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-blue-500" onChange={(e) => setFormData({...formData, confirmPassword: e.target.value})} />
              </div>
            )}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col sm:flex-row text-left">
      {/* Sidebar */}
      <div className="w-full sm:w-72 bg-white border-r p-8 flex flex-col shadow-sm">
        ...
        <nav className="space-y-2 flex-1">
          <button className="w-full flex items-center gap-3 text-blue-600 font-bold bg-blue-50 p-4 rounded-2xl"><Users className="w-5 h-5" /> Contacts</button>
          <div className="mt-8 pt-8 border-t">
             <div className="bg-slate-900 rounded-3xl p-6 text-white">
                <Crown className="w-8 h-8 text-yellow-400 mb-3" />
                <h3 className="font-bold mb-1 text-left">Premium Trial</h3>
                <p className="text-xs text-slate-400 text-left">Unlimited tracking active.</p>
             </div>
          </div>
        </nav>
        <button onClick={handleLogout} className="mt-auto w-full flex items-center gap-2 text-red-500 hover:bg-red-50 p-4 rounded-2xl font-bold transition">
      ...
      <div className="flex-1 p-6 sm:p-12 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {contacts.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase())).map(contact => (
              <div key={contact.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all group">
        ...
        {showAddForm && (
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-6 z-50">
            <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl text-left">
              <h2 className="text-3xl font-black text-slate-900 mb-8">Add Relationship</h2>
              <form onSubmit={handleAddContact} className="space-y-6">
                <input type="text" placeholder="Client Name" required className="w-full p-5 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 outline-none" onChange={(e) => setFormData({...formData, name: e.target.value})} />
                <input type="email" placeholder="Email Address" className="w-full p-5 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 outline-none" onChange={(e) => setFormData({...formData, email: e.target.value})} />
                <div>
